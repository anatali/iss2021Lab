...
class receiver(name: String, scope: CoroutineScope) : ActorBasicFsm( name, scope ){
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		return {
				state("init") {
					action {
						println("receiver | STARTS")
                        //forward a dispatch to itself
                        forward("msg1", "msg1(autoHello)" ,"receiver" ) 
						//TODO 
					}
 				...
 				state("waitinput") {
					action {
                        stateTimer = TimerActor("timer_waitinput", 
                             scope, context!!, "local_tout_receiver_waitinput", 1000.toLong() )
					}
                    transition(edgeName="t00",targetState="handleTimeOut", cond=whenTimeout("local_tout_receiver_waitinput")) 
				...
				state("handleRequest") { //this:State
					action { //it:State
						println("receiver in handleRequest")
						println("$name in ${currentState.stateName} | $currentMsg")
					}
					 transition( edgeName="goto",targetState="waitinput", cond=doswitch() )
				}	 
				...
				state("handleAlarm") { //this:State
					action { //it:State
						println("receiver in handleAlarm")
						println("$name in ${currentState.stateName} | $currentMsg")
					}
