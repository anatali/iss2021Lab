...
public class CautiousRobotInputController implements IssObserver {
private CautiousRobotLogic robotBehaviorLogic  ;
private IssCommSupport commSupport;

private boolean robotStarted = false;
private boolean robotHalted  = true;

    public CautiousRobotInputController(IssCommSupport support, boolean usearil, boolean doMap){
        commSupport = support;
        robotBehaviorLogic = new CautiousRobotLogic(support, usearil, doMap);
     }
...
Handler of the robotcmd (STOP/RESUME) messages sent by the consoleGui
    ...
    protected void handleEndMove(JSONObject endmove ){
        ...
        switch( answer ){
            case "true"       : robotBehaviorLogic.nextStep( move,false, robotHalted);
                                break;
            case "false"      : robotBehaviorLogic.backStep( move, true, robotHalted  );
                                break;
